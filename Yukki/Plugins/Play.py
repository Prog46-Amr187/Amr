...
async def play(_, message: Message):
    ...
    if audio:
        ...
        if (audio.duration) > DURATION_LIMIT:
            return await mystic.edit_text(
                f"**تم تجاوز حد المدة**\n\n**المدة المسموح بها: **{DURATION_LIMIT_MIN} دقيقه\n**المدة المستلمة:** {duration_min} دقيقه"
            )
        file_name = (
        ...
        if len(message.command) < 2:
            ...
            await message.reply_photo(
                ...
                caption=(
                    "**استخدم:** /play [وجانبها اسم الاغنيه ، الرابط ، الرد علي الملف]\n\nاستخدام احد الاخيارات السابقه اذا كنت تريد تشغيل الموسيقى في المحادثة الصوتيه."
                ),
                reply_markup=InlineKeyboardMarkup(buttons),
            )
            return
...
async def startyuplay(_, CallbackQuery):
    ...
    if CallbackQuery.from_user.id != int(user_id):
        return await CallbackQuery.answer(
            "هذا ليس لك! ابحث في الأغنية الخاصة بك.", show_alert=True
        )
